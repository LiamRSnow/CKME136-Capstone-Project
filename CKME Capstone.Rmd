...
#Visualizing Data
ggplot(data=Divvy2018, aes(Divvy2018$tripduration))+geom_histogram(binwidth=150,fill="#B3DDF2")+scale_x_continuous(name = "Trip Duration (s)")+scale_y_continuous(name="Trips in 2018",labels=scales::comma,limits=c(0,600000))+ggtitle("Frequency of Trip Duration")+theme(panel.background = element_rect(fill = 'grey44'))
...
ggplot(aggmonth,aes(x=aggmonth$month,y=aggmonth$tripduration))+geom_bar(stat="identity",fill="#B3DDF2")+xlim(c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"))+theme(panel.background = element_rect(fill = 'grey44'))+ggtitle("AverageTrip Perentage per Month")+xlab("")+ylab("Average Trip Duration (s)")

aggdow=aggregate(Divvy2018,by=list(Divvy2018$dayofweek),FUN=mean)
aggdow$dayofweek=c("Fri","Mon","Sat","Sun","Thu","Tue","Wed")
ggplot(aggdow,aes(x=aggdow$dayofweek,y=aggdow$tripduration))+geom_bar(stat="identity",fill="#B3DDF2")+xlim(c("Sun","Mon","Tue","Wed","Thu","Fri","Sat"))+theme(panel.background = element_rect(fill = 'grey44'))+ggtitle("Trip Perentage per Month")+xlab("")+ylab("Average Trip Duration (s)")
